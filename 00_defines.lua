...
NDiplomacy = {
	...
	MIN_WARGOAL_JUSTIFY_COST = 0.5,					-- It always takes atleast 10 days to justify a wargoal
	WARGOAL_PER_JUSTIFY_AND_WAR_COST_FACTOR = 0.25,	-- Cost factor per nation at war with or justifying against
	BASE_BOOST_PARTY_POPULARITY_DAILY_PP = 0.5, 	-- Daily pp cost for boost party popularity
	BASE_BOOST_PARTY_POPULARITY_DAILY_DRIFT = 0.2, 	-- Daily amount of popularity that will be added by the activity.
	...
	TENSION_ANNEX_CORE = 0.1,						-- Amount of tension generated by annexing a state that is your core
	TENSION_PUPPET = 0.5,								-- Amount of tension generated by puppeting (per state)
	TENSION_GENERATE_WARGOAL = 2,					-- Amount of tension generated by generating a wargoal against a country
	...
	TENSION_TIME_SCALE_START_DATE = "1945.1.1.12",	-- Starting at this date, the tension values will be scaled down (will be equal to 1 before that)
	TENSION_TIME_SCALE_MONTHLY_FACTOR = -0.02,		-- Timed tension scale will be modified by this amount starting with TENSION_TIME_SCALE_START_DATE
	TENSION_TIME_SCALE_MIN = 0.00,					-- Timed tension scale won't decrease under this value
	TENSION_GUARANTEE = -5,
	...
	IDEOLOGY_JOIN_FACTION_MIN_LEVEL = 0.6,			-- ideology limit required to join faction
...
NCountry = {
	...
	NUCLEAR_BOMB_DROP_UNITY_EFFECT_MAX_INFRA = 0.4, -- Reduce enemy national unity on nuking a province, the value scales with infrastructure up to this number
...
NTechnology = {
	...
	BASE_RESEARCH_POINTS_SAVED = 300.0,		-- Base amount of research points a country can save per slot.
...
NBuildings = {
	...
	DESTRUCTION_COOLDOWN_IN_WAR = 7,	-- Number of days cooldown between removal of buildings in war times
},
...
NMilitary = {
	...
	WAR_SCORE_LOSSES_RATIO = 0.0001,				-- Scale how much losses will affect warscore ( per 1000 losses ).
	WAR_SCORE_LOSSES_MULT_IF_CAPITULATED = 0.0, -- How much your losses count towards warscore if you have capitulated.
	WAR_SCORE_AIR_WORTH = 0.01,              	-- how much strategic bombing will affect warscore
	WAR_SCORE_AIR_WORTH_CAP = 1000,				-- the warscore limit we get from strategic bombing
	WAR_SCORE_AIR_MONTHLY_FALLOFF = 1,			-- how much the warscore we got from the strategic bombing falls off every month.
	...
	MAX_DIVISION_BRIGADE_HEIGHT = 6,		-- Max height of regiments in division designer.
	...
	MAX_ARMY_EXPERIENCE = 50000,			--Max army experience a country can store
	MAX_NAVY_EXPERIENCE = 50000,			--Max navy experience a country can store
	MAX_AIR_EXPERIENCE = 50000,				--Max air experience a country can store
	...
	LAND_COMBAT_ORG_DAMAGE_MODIFIER = 0.03,        -- global damage modifier
	...
	LAND_AIR_COMBAT_MAX_PLANES_PER_ENEMY_WIDTH = 50, -- how many CAS/TAC can enter a combat depending on enemy width there
	...
	BASE_CHANCE_TO_AVOID_HIT = 97,                 -- Base chance to avoid hit if defences left.
	CHANCE_TO_AVOID_HIT_AT_NO_DEF = 20,	           -- chance to avoid hit if no defences left.
	...
	WARSCORE_PROVINCE_FACTOR = 0.4,					-- Warscore effect from province
	...
	FIELD_EXPERIENCE_MAX_PER_DAY = 300,				-- Most xp you can gain per day
	...
	PARACHUTE_PENALTY_RANDOMNESS = 0.05,			   -- Random factor for str,manpower,eq penalties.
	...
	OUT_OF_SUPPLY_MORALE = -0.5,                   -- max org regain reduction from supply
	...
	PLAN_MIN_AUTOMATED_EMPTY_POCKET_SIZE = 1,		-- The battle plan system will only automatically attack provinces in pockets that has no resistance and are no bigger than these many provinces
	...
	PLAN_BLITZ_OPTIMISM = 0.5,						-- Additional combat balance value in favor of blitzing side when considering targets (not a combat bonus, just offsets planning)
...
NNavy = {
	...
	EXPERIENCE_LOSS_FACTOR = 0.9,                 					-- percentage of experienced solders who die when manpower is removed
...
NAI = {
	...
	DIPLOMACY_FACTION_WRONG_IDEOLOGY_PENALTY = 600, -- AI penalty for diplomatic faction acitons between nations of different ideologies
	...
	DIPLOMACY_FACTION_NEUTRALITY_PENALTY = 500,	-- Neutral nations have a separate penalty, not wanting to get involved at all, rather than caring much about the difference in ideology
	...
	DIPLOMACY_SCARED_MINOR_EXTRA_RELUCTANCE = -500, -- extra reluctance to join stuff as scared minor
	DIPLOMACY_FACTION_PLAYER_JOIN = 50,			-- Bonus for human players asking to join a faction.
	...
	DIPLOMACY_FACTION_MAJOR_AT_WAR = 1.0,	-- Factor that will be multiplied with the surrender level in the desire to offer to the other ai to join a faction
	...
	UPGRADE_XP_RUSH_UPDATE = 5000,				-- If XP is above this on the daily tick the AI will attempt to spend it
	...
	PLAN_FRONTUNIT_DISTANCE_FACTOR = 20.0,		-- Factor for candidate units distance to front positions.
	...
	IRRATIONALITY_LAMBDA = 600,					-- Lambda given to Poisson Random function determining if a leader should act a bit irrational 											
												-- Value of 200 should give 0.3% chance of Stalin going for instance crazy and conquering all of America
	GENERATE_WARGOAL_THREAT_BASELINE = 0.5,		-- The baseline for what the AI considers the world is getting dangerous and we want to generate wargoals	
	...
	TENSION_MIN_FOR_GUARANTEE_VS_MINOR = 90, -- for non faction people AI will not consider you worth guaranteeing below this
	
	NUM_AI_MESSAGES = 10,				-- Set to whatever category has the highest number of messages

	DIPLOMACY_FACTION_WAR_WANTS_HELP = -500,	-- Desire to send to nations to join a faction if you are at war
	...
	UPGRADE_DIVISION_RELUCTANCE = 2,					-- How often to consider upgrading to new templates for units in the field, was 7
	...
	HOUR_BAD_COMBAT_REEVALUATE = 8,                   -- if we are in combat for this amount and it goes shitty then try skipping it 
	MIN_PLAN_VALUE_TO_MICRO_INACTIVE = 0.05,				-- The AI will not consider members of groups which plan is not activated AND evaluates lower than this.
	...
	AVERAGE_SUPPLY_USE_PESSIMISM = 1.3,					-- Multiplier for when AI calculates average supply use of entire army.
	...
	INVASION_DISTANCE_RANDOMNESS = 500,					-- This higher the value, the more unpredictable the invasions. Compares to actual map distance in pixels.
